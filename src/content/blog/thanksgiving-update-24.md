...
- Files cannot be opened as **read and write** at the same time (only one). To read a file that you've written to, you'll need to drop your written file descriptor reopen it as readonly.
...
> [!IMPORTANT]
> We've also fixed a pretty serious bug where ADI devices would not update if they were written to at intervals faster than 10 milliseconds.
