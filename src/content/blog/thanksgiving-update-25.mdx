...
> [!TIP]
> vexide is a Rust library for programming VEX robots, empowering students to write safe and efficient code for competitive robots in the Rust programming language. [Learn more.](/docs/#what-is-vexide)
...
This looks a lot like a normal static, but when we spawn an [async task](/docs/async-introduction/), each task will get its own unique version of `COUNT` starting at `1`. In other words, value of `COUNT` is *local* to each task accessing it.
...
We've overhauled the Addressable LED API ([`AdiAddrLed`](https://docs.rs/vexide/0.8.0/vexide/adi/addrled/struct.AdiAddrLed.html)) for this release to allow for controlling LED strips from vexide without allocating data on the heap. Here's an example of that in action:
...
> [!IMPORTANT]
> Please read our [migration guide](/blog/posts/080-migration-guide/) for detailed instructions on updating your project.
...
Error: [31mcargo_v5::radio_channel_stuck[0m
  ...
  [31m×[0m Controller is stuck in radio channel 9.
...
- [GamingLiamStudios](https://github.com/GamingLiamStudios) fixed several issues with our filesystem implementation.
- [fibonacci61](https://github.com/fibonacci61) wrote an initial implementation of our new async Task-local Storage system in vexide's async runtime.
- [slipperking](https://github.com/slipperking) fixed some bugs in vexide's AI Vision API and helped with documentation.
- [aadishv](https://github.com/aadishv) and the [`venice-v5` team](https://github.com/venice-v5/venice) provided important feedback and bug reports for vexide 0.8.0 throughout its development.
