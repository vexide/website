...
A [panic](https://doc.rust-lang.org/book/ch09-01-unrecoverable-errors-with-panic.html) is an *intentional decision* made by the program to stop running due to some unrecoverable or unhandled error. Panics can occur for a variety of reasons, but typically happen when something goes wrong at the logic or runtime level. For example:
...
An **abort** (also called an *exception*) is a serious, unexpected failure that happens when the Brain's user processor experiences a hardware or system-level error. Aborts are often a sign of *undefined behavior* from `unsafe` code being misused (either from your code or from a library you're using).
...
## Tracing Abort Addresses

As mentioned previously, aborts are more difficult to trace back to a line of code. Unlike panics, aborts do not give us a file name or line number. Instead we are given the memory address of the instruction that caused the abort. To resolve this address to a location in our source code, we have to use a tool called a *symbolizer*.

> [!TIP]
> This page covers a command-line method of symbolizing addresses. If you use VSCode, we have an extension that nicely automates the whole process for you. Follow [this guide](https://github.com/vexide/symbolizer-for-vex-v5/wiki/Find-where-your-code-is-crashing) instead.

A symbolizer translates an instruction address into a human-readable location using debug information from your compiled code. In this tutorial, we'll use the `llvm-symbolizer` tool. If you don't have LLVM installed, go download it [here](https://releases.llvm.org/).

To find the location of our abort, we need two things:
1. The address of the abort. This is printed to the terminal and shown on the display as a hexadecimal number when the abort occurs.
2. Our compiled code. This *must* be the exact same version of the code that was subsequently uploaded to the Brain when the abort happened. You can typically find this in your project's `target` folder at `./target/<profile>/<name>` as a file with no extension. Replace `<profile>` with either `debug` or `release` depending on if your project was compiled in debug or release mode and `<name>` with the name of your project.

To use `llvm-symbolizer`, we give it our compiled code and the address of the abort. Let's say we aborted at address `0x123456678` in a project named `my-robot` compiled for `release` mode:

<SplitCodeblock vertical>

```sh
llvm-symbolizer -p -o ./target/release/my-project 0x12345678
```

```ansi
src/main.rs:42:13
my_crate::module::some_function
```
</SplitCodeblock>

Running this should hopefully give you the file name, line number, and symbol name where the abort occurred.